...
### 后台
- ✅ 文章管理（表格布局、分页、内联新建/编辑）
- ✅ 分类管理（表格布局）
- ✅ 回收站（表格布局、恢复/彻底删除）
- ✅ 网站设置（网站标题、图标、主题、页脚、自定义JS、功能开关）
- ✅ 个人设置（头像、简介、友链标题/内容）
- ✅ 主题切换（动物森林 / 海洋微风）
- ✅ 图片上传（支持上传 + 外链，限制 1MB）
- ✅ 响应式布局（手机端 / 平板端 / 桌面端）
...
### 安全
- ✅ HMAC-SHA256 认证（48小时过期）
- ✅ 登录速率限制（5次/10分钟）
- ✅ XSS 防护（Markdown 内容转义）
- ✅ SQL 注入防护（参数化查询）
- ✅ 文件上传限制（1MB、类型验证）
- ✅ 错误信息隐藏（仅记录到服务器日志）
- ✅ CORS 可配置来源
...
```
src/
├── worker.js              # 主入口（路由分发、robots.txt、favicon.ico）
├── api.js                 # API 处理（分页、缓存、输入验证、速率限制）
├── lib/
│   ├── utils.js           # 工具函数（JSON 响应、HTML 转义、CORS）
│   ├── db.js              # 数据库初始化（索引、批量操作）
│   ├── auth.js            # HMAC-SHA256 认证（48h 过期）
│   ├── cache.js           # Workers Cache API
│   └── image.js           # 图片处理（R2 上传、1MB 限制、类型验证）
└── views/
    ├── frontend.js        # 前台首页（SEO、分页、响应式）
...
| `ALLOWED_ORIGIN` | Variable | CORS 允许的来源（如 `https://你的域名`，默认 `*`） |
